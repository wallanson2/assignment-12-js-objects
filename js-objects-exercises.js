...
function flipColor(inpObject) {

  if(inpObject.color === "blue"){
    inpObject.color = "red";
  } else if (inpObject.color === "red"){
    inpObject.color = "blue";
  }
  console.log(inpObject.color)
return inpObject
}
...
function getFullNames(names){
    ...
    for (var i = 0; i < customers.length; i += 1){
        fullNames[i] = customers[i].first + " " + customers[i].last;
    }
	return fullNames
}
...
//  INPUT: Array of full names==========

function createListOfObjects(ninjas) {
  var newNinjasList = []
  for (var i = 0; i < ninjas.length; i += 1){
    var seperatedNames = ninjas[i].split(' ')
    var newNinjasObj ={
      firstName: seperatedNames[0],
      lastName: seperatedNames[1]
    }
      newNinjasList.push(newNinjasObj)

  }
  return newNinjasList
}



//OUTPUT: Array of objects  with first and last name properties
...
// pluck(stooges, 'name') should yield the array, ['moe','larry','curly']
   ...
   function pluck(familyGroup, str){
       var newArray = [];
       for (var i = 0; i < familyGroup.length; i += 1){
           newArray.push(familyGroup[i][str]);
       }
     return newArray;
   }
...
function getCounts(str) {
  ...
  for ( var i = 0; i < splitArr.length; i += 1) {
    var theWord = splitArr[i];
    if( typeof wordFrequencyObject[theWord] === "undefined") {
      wordFrequencyObject[theWord] = 1;
    } else {
      wordFrequencyObject[theWord] = wordFrequencyObject[theWord] + 1;
    }
  }
